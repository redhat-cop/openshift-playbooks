...
function launchsentinel() {
  ...
  echo "${master} Sentilel started" >> /redis-sentinel-data/log
}
...
function launchslave() {
  ...
  cp /redis-slave/redis.conf /redis-master-data/redis.conf
#  perl -pi -e "s/%master-ip%/${master}/" /redis-slave/redis.conf
  perl -pi -e "s/%master-ip%/${master}/" /redis-master-data/redis.conf
#  perl -pi -e "s/%master-port%/6379/" /redis-slave/redis.conf
  perl -pi -e "s/%master-port%/6379/" /redis-master-data/redis.conf
  redis-server /redis-master-data/redis.conf
}
