...
Sizing for Master nodes is calculated based on the number of pods the cluster is expected to host. The calculation guidelines can be found in the link:{docs_url}install_config/install/prerequisites.html#host-recommendations[Host Recommendations section in the install guide].

== Sizing for App Nodes

The Offcial Documentation lists some link:{docs_url}install_config/install/prerequisites.html#system-requirements[minimal system requirements] for OpenShift masters and nodes. Beyond that, there are several things to consider when deciding on the sizing for your nodes.
