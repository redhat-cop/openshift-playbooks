...
Under *Endpoints*, in the *Default* tab, enter the cluster public hostname (openshift_master_cluster_public_hostname in `/etc/ansible/hosts`) into the *Hostname* field and enter the token retrieved earlier into the *Token* and *Confirm Token* fields. Click *Validate* to confirm connectivity.
...
Capacity and Utilization metrics will begin to flow into CFME and will take approximately 24 hours to process data and make the default report generation and visualizations available.  For chargeback reports, it takes approximately a total of 48 hours for the report data to become available.  Additionally, chargeback data is not available for the current day (only prior day, 24 hour lag) because it has not been analyzed yet.
