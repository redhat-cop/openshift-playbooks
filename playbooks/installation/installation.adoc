...
Base OS: RHEL 7.2 or later with "Minimal" installation option, or RHEL Atomic Host 7.2.4 or later.
...
* Install Red Hat provided Red Hat Enterprise Linux 7.2 image
...
* Install the following extra packages: `yum install wget git net-tools bind-utils iptables-services bridge-utils rpcbind nfs-utils`
...
== Post Installation Openshift Configuration

=== Isolating InfraNodes Regions

[source, bash]
----
for n in $)( oc get namespace |grep -v NAME|awk {'print $1'});do
oc annotate --overwrite namespace $n openshift.io/node-selector='region=infra'
done
----

=== Deploying Router with Custom Certificates
...
oadm router router \
   --default-cert=/path/to/cert/splat.example.com.pem
...
[source, bash]
----
oadm groups new group-name
----

Adding users to a group:
[source, bash]
----
oadm groups add-user group-name user1 user2 user3
----
...
[source, bash]
----
oadm add-role-togroup view group-name
----
