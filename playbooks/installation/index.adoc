...
* 3 _Masters_ (openshift-master-[1-3].c1-ocp.myorg.com)
  ...
  ** 50 GB+ - `/var/lib/docker` (If using Docker as the container runtime, the container image cache formatted, configured, and managed by `docker-storage-setup`.  In this guide, available as `/dev/vdc`. It is recommended that `STORAGE_DRIVER=overlay2` be set in `/etc/sysconfig/docker-storage-setup`.)
  ** 50 GB+ - `/var/lib/containers` (If using CRI-O as the container runtime, the container image cache formatted, configured, and managed by `container-storage-setup`.  In this guide, available as `/dev/vdc`. It is recommended that `storage_driver=overlay` be set in `/etc/crio/crio.conf`.)
...
* 3 _Infrastructure Nodes_ (openshift-infra[1-3].c1-ocp.myorg.com)
  ...
  ** 10 GB - `/usr` (system binaries)
  ** 10 GB - `/var` (system drivers and other bits)
  ** 100 GB+ - `/var/lib/docker` (If using Docker as the container runtime, the container image cache formatted, configured, and managed by `docker-storage-setup`.  In this guide, available as `/dev/vdc`. It is recommended that `STORAGE_DRIVER=overlay2` be set in `/etc/sysconfig/docker-storage-setup`.)
  ** 50 GB+ - `/var/lib/containers` (If using CRI-O as the container runtime, the container image cache formatted, configured, and managed by `container-storage-setup`.  In this guide, available as `/dev/vdc`. It is recommended that `storage_driver=overlay` be set in `/etc/crio/crio.conf`.)
  ** 50 GB+ - `/var/lib/origin` (Ephemeral/temp storage for running containers.  Should be backed by a logical volume.)
  ** 20 GB+ - `/var/log` (system logging write path)
  ** `[SWAP]` (should be disabled and not partitioned for)
