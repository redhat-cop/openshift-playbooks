...
```
ansible -i c1-ocp.myorg.com/hosts nodes -m shell -a 'rpm -ivh http://satellite6.c1-ocp.myorg.com/pub/katello-ca-consumer-latest.noarch.rpm'
ansible -i c1-ocp.myorg.com/hosts nodes -m shell -a 'subscription-manager register --org="<My_Org>" --activationkey="<my-activation-key>"'
```
...
----
ansible -i c1-ocp.myorg.com/hosts OSEv3 -m shell -a 'subscription-manager register --username bob@acme.com --password='mypassword'
ansible -i c1-ocp.myorg.com/hosts OSEv3 -m shell -a 'subscription-manager attach --pool=8a85f98144844aff014488d058bf15be'
ansible -i c1-ocp.myorg.com/hosts OSEv3 -m shell -a 'subscription-manager repos --disable="*" --enable="rhel-7-server-rpms" --enable="rhel-7-server-extras-rpms" --enable="rhel-7-server-ose-3.6-rpms" --enable="rhel-7-fast-datapath-rpms'
----
...
----
ansible -i c1-ocp.myorg.com/hosts nodes -m shell -a 'echo "DEVS=/dev/vdb" > /etc/sysconfig/docker-storage-setup'
----
...
----
ansible -i c1-ocp.myorg.com/hosts etcd -m shell -a 'yum -y install lvm2'

ansible -i c1-ocp.myorg.com/hosts etcd -m shell -a 'pvcreate /dev/vdc'
ansible -i c1-ocp.myorg.com/hosts etcd -m shell -a 'vgcreate etcd-vg /dev/vdc'
ansible -i c1-ocp.myorg.com/hosts etcd -m shell -a 'lvcreate -n etcd-lv -l 100%VG etcd-vg'
ansible -i c1-ocp.myorg.com/hosts etcd -m shell -a 'mkfs.xfs /dev/mapper/etcd--vg-etcd--lv'
...
ansible -i c1-ocp.myorg.com/hosts nodes:!etcd -m shell -a 'yum -y install lvm2'

ansible -i c1-ocp.myorg.com/hosts nodes:!etcd -m shell -a 'pvcreate /dev/vdc'
ansible -i c1-ocp.myorg.com/hosts nodes:!etcd -m shell -a 'vgcreate origin-vg /dev/vdc'
ansible -i c1-ocp.myorg.com/hosts nodes:!etcd -m shell -a 'lvcreate -n origin-lv -l 100%VG origin-vg'
ansible -i c1-ocp.myorg.com/hosts nodes:!etcd -m shell -a 'mkfs.xfs /dev/mapper/origin--vg-origin--lv'
...
openshift_node_kubelet_args:
  kube-reserved:
  - "cpu=250m,memory=500M" <1>
  system-reserved:
  - "cpu=250m,memory=500M" <2>
  eviction-hard:
  - "memory.available<100Mi" <3>
  ...
  - '60'<10>
----
<1> Resources reserved for node components
<2> Resources reserved for the remaining system components
<3> Value the node attempts to evict pods whenever memory availability on the node drops below the absolute value
<4> The minimum age that a container is eligible for garbage collection
<5> The number of instances to retain per pod container
<6> node removes the dead containers, all files inside those containers are removed as well
<7> How many pods will be allowed to run per core
<8> Max number of pods on a node
<9> The percent of disk usage which triggers image garbage collection
<10> The percent of disk usage to which image garbage collection attempts to free
