...
Just as with the Docker Storage, during the link:#provision-servers[Provision Servers] step of this guide, we provisioned all of our nodes (including the masters) with an extra volume to be used for `etcd`, attached as `/dec/vdc`. Make sure to replace this with the disk available in your environment. We will now demonstrate the steps involved with using LVM to set up and use that volume for etcd backed storage. 
