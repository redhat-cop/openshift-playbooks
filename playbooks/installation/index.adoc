...
----
hammer product create --name "OCP Docker Images" --organization "c1-ocp.myorg.com"
hammer repository create --name "openshift3/ose" --organization-id "c1-ocp.myorg.com" --content-type docker --url "https://registry.access.redhat.com" --docker-upstream-name "openshift3/ose" --product "OCP Docker Images"
----

NOTE: Satellite will sync ALL tags for each image repository you create so it might be space intensive.

There is a script available at https://github.com/redhat-cop/openshift-toolkit/tree/master/satellite/populate-ocp.sh that can be used to save time.
...
cli_docker_additional_registries=satellite.example.com:5000
----
