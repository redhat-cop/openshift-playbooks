...
Write this content to a file called `c1-ocp.myorg.com/hosts`. We'll add to this file as we go.
...
ssh will require you to accept the new ssh key for the first time into the `~/.ssh/known_hosts` file by either ssh'ing into each of the nodes one by one and typing yes each time or adding the file `~/.ssh/config` file with perms of 600 with a line that includes `StrictHostKeyChecking no`.  Once this is completed you can test that ansible will no longer ask to accept the key
...
openshift_docker_additional_registries=registry.c1-ocp.myorg.com
openshift_docker_insecure_registries=registry.c1-ocp.myorg.com
...
```
ansible -i repository/playbooks-ocplabcluster nodes -a 'rpm -ivh http://satellite6.c1-ocp.myorg.com/pub/katello-ca-consumer-latest.noarch.rpm'
...
----
ansible -i hosts nodes -a 'echo "DEVS=/dev/vdb" > /etc/sysconfig/docker-storage-setup'
----
...
Just as with the Docker Storage, during the link:#provision-servers[Provision Servers] step of this guide, we provisioned our masters and nodes with an extra volume to be used for `etcd` (for masters) and `/var/lib/origin` (for nodes), attached as `/dev/vdc`in this guide. (Make sure to replace this with the disk available in your environment.) We will now demonstrate the steps involved with using LVM to set up and use this volume for backing storage. 

----
ansible -i hosts etcd -a 'yum -y install lvm2'

ansible -i hosts etcd -a 'pvcreate /dev/vdc'
ansible -i hosts etcd -a 'vgcreate etcd-vg /dev/vdc'
ansible -i hosts etcd -a 'lvcreate -n etcd-lv -l 100%VG etcd-vg'
ansible -i hosts etcd -a 'mkfs.xfs /dev/mapper/etcd--vg-etcd--lv'
ansible -i hosts etcd -m shell -a 'mkdir /var/lib/etcd'
ansible -i hosts etcd -m lineinfile -a 'path=/etc/fstab regexp=etcd line="/dev/mapper/etcd--vg-etcd--lv /var/lib/etcd xfs defaults 0 0"'
ansible -i hosts etcd -m shell -a 'mount -a'

ansible -i hosts nodes -a 'pvcreate /dev/vdc'
ansible -i hosts nodes -a 'vgcreate origin-vg /dev/vdc'
ansible -i hosts nodes -a 'lvcreate -n origin-lv -l 100%VG origin-vg'
ansible -i hosts nodes -a 'mkfs.xfs /dev/mapper/origin--vg-origin--lv'
ansible -i hosts nodes -m shell -a 'mkdir /var/lib/origin'
ansible -i hosts nodes -m lineinfile -a 'path=/etc/fstab regexp=origin line="/dev/mapper/origin--vg-origin--lv /var/lib/origin xfs defaults 0 0"'
ansible -i hosts nodes -m shell -a 'mount -a'
----
