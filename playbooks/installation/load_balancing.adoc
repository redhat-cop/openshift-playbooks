...
TIP: By default, the use of a Ramp Node creates a single point of failure. To avoid this, you must configure a link:https://docs.openshift.org/latest/install_config/routing_from_edge_lb.html#configuring-a-highly-available-ramp-node[highly available ramp node].
...
* Domain Name for VIP registered in DNS
  ...
  ** For this example, we will give our VIP a domain name: _paas-internal.myorg.com_
* Master hosts created and prepped per link:https://docs.openshift.com/enterprise/3.1/install_config/install/prerequisites.html[Install Prerequisites]
  ...
  ** For this example, we will give our masters the following hostnames: _master01.myorg.com_, _master02.myorg.com_, _master03.myorg.com_
  ** Fir this example, we will give our masters the following IP addresses: 192.168.122.1, 192.168.122.2, 192.168.122.3
...
oadm ca create-master-certs \
  ...
  --master=paas-internal.myorg.com \
  --public-master=paas-internal.myorg.com \
