...
At its core, OpenShift Applier is an link:https://ansible.com[ansible role] that creates an ansible inventory syntax for automating the rollout of a set of templates and parameter files. What this does is greatly reduces the level of effort to build and maintain quality automation of OpenShift resources.
...
Your hosts file would simply contain:
