...
There are two main components of OpenShift that require specialized storage considerations to ensure environment stability. These components are Docker and etcd. Without provisioning a separate logical volume for these components, OS logs could potentially fill the disk, preventing either of these from writing to disk and corrupting the OSE install.
...
Create the following partitions on the system disk image, with a minimum of 40GB unallocated:

- Boot partition
- OS Root

From the unallocated space, create the following logical volumes:
